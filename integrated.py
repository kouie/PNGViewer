...
class MetadataLabel(QLabel):
    ...
    def contextMenuEvent(self, event):
        ...
        selectAllAction = menu.addAction("項目の値をコピー"); selectAllAction.triggered.connect(self.selectAll)
        menu.addSeparator()
        sendAction = menu.addAction("👉 この項目を生成パネルにセット"); sendAction.triggered.connect(lambda: self.send_single_to_forge.emit(self.label, self.value, False))
        sendGenAction = menu.addAction("🚀 この項目をセットして生成スタート"); sendGenAction.triggered.connect(lambda: self.send_single_to_forge.emit(self.label, self.value, True))
        menu.addSeparator()
        ...
        menu.addSeparator()
        customAction = menu.addAction("項目を選択..."); customAction.triggered.connect(self.on_r_mouse_clicked); menu.exec(event.globalPos())
...
class CheckableListDialog(QDialog):
    def __init__(self, items, parent=None):
        super().__init__(parent); self.setWindowTitle("項目選択"); self.resize(180, 400); self.layout = QVBoxLayout(self); self.checkboxes = []
        for item in items:
...
class ImageView(QWidget):
    ...
    def setup_toolbar(self):
        ...
        self.combo_sort = NoWheelComboBox(); self.combo_sort.addItems(["日付順", "名前順"]); self.combo_sort.currentIndexChanged.connect(self.refresh_folder_view); toolbar.addWidget(self.combo_sort)
    ...
    def show_tagSelection_ContextMenu(self, position):
        ...
        selectAction = QAction("項目を選択...", self); selectAction.triggered.connect(self.selectItems); menu.addAction(selectAction)
    ...
    def show_slider_popup(self, position):
        ...
        self.slider_popup.slider.blockSignals(True); self.slider_popup.slider.setMaximum(len(image_files) - 1); self.slider_popup.image_files = image_files.copy()
        current_name = os.path.basename(self.current_image_path)
        if current_name in image_files: self.current_index = image_files.index(current_name)
        self.slider_popup.slider.setValue(self.current_index); self.slider_popup.slider.blockSignals(False); self.update_page_display(len(image_files))
        if position is not None:
            popup_pos = QCursor.pos() + QPoint(-self.slider_popup.width() // 2, 10); screen_rect = QGuiApplication.primaryScreen().geometry()
            popup_pos.setX(max(screen_rect.left(), min(popup_pos.x(), screen_rect.right() - self.slider_popup.width())))
            self.slider_popup.move(popup_pos); self.slider_popup.show()
    ...
    def clear_view_area(self, text):
        self.image_label.setText(text); self.image_label.setPixmap(QPixmap()); self.current_image_path, self.image_label.image_path = "", ""
    ...
    def change_image(self, event):
        ...
        if self.slider_popup.chk_filter.isChecked():
            ...
            if not (files := self.get_sorted_image_files(apply_filter=False)): return
            current_idx = files.index(os.path.basename(self.current_image_path)) if os.path.basename(self.current_image_path) in files else 0
                    ...
                    try:
                        ...
                        for _ in range(len(files)):
                            if regex.search(self.extract_png_metadata(os.path.join(self.current_folder, files[new_idx])).get("Negative prompt" if is_neg else "Prompt", "")):
                                self.load_image(os.path.join(self.current_folder, files[new_idx])); return
            ...
            try:
                ...
                for _ in range(len(files)):
                    if regex.search(self.extract_png_metadata(os.path.join(self.current_folder, files[current_idx])).get("Negative prompt" if is_neg else "Prompt", "")):
                        self.load_image(os.path.join(self.current_folder, files[current_idx])); return
...
class ForgeClientPanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.btn_start = QPushButton("▶️ 生成スタート"); self.btn_start.setStyleSheet("background-color: #4CAF50; color: white; font-size: 14px; font-weight: bold; padding: 10px;"); self.btn_start.clicked.connect(self.start_generation)
        self.btn_stop_loop = QPushButton("⏹️ ループ停止"); self.btn_stop_loop.setStyleSheet("background-color: #FF9800; color: white; font-weight: bold;"); self.btn_stop_loop.setEnabled(False); self.btn_stop_loop.clicked.connect(self.stop_loop_only)
        self.btn_interrupt = QPushButton("🛑 中断"); self.btn_interrupt.setStyleSheet("background-color: #f44336; color: white; font-weight: bold;"); self.btn_interrupt.setEnabled(False); self.btn_interrupt.clicked.connect(self.stop_and_interrupt)
        btn_layout.addWidget(self.btn_start, stretch=3); btn_layout.addWidget(self.btn_stop_loop, stretch=1); btn_layout.addWidget(self.btn_interrupt, stretch=1); bottom_layout.addLayout(btn_layout)

        prompt_group = QGroupBox("1. プロンプト ( Ctrl+↑/↓ 強調, {a|b} 展開 )"); prompt_layout = QVBoxLayout(prompt_group)
        ...
        param_layout.addWidget(QLabel("Seed:"), 4, 0); self.edit_seed = QLineEdit("-1"); param_layout.addWidget(self.edit_seed, 4, 1, 1, 3)
        btn_random_seed = QPushButton("🎲 -1"); btn_random_seed.clicked.connect(lambda: self.edit_seed.setText("-1")); param_layout.addWidget(btn_random_seed, 4, 4)
        ...
        model_layout.addWidget(self.combo_model, stretch=1); model_layout.addWidget(btn_refresh_models); top_g_layout.addLayout(model_layout); bottom_layout.addWidget(top_group)
    ...
    def start_generation(self):
        ...
        self.thread.status_updated.connect(self.lbl_status.setText); self.thread.image_generated.connect(self.on_image_generated); self.thread.error_occurred.connect(self.on_generation_error); self.thread.finished_all.connect(self.on_generation_finished)
        self.thread.start(); self.progress_timer.start(1000)
    ...
    def check_forge_progress(self):
        try:
            ...
            if res.status_code == 200:
                ...
                step_val = int((progress_val * 100) // 25) * 25
